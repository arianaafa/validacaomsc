...
final class D2_00002Rule implements MscFileFinalizerRuleInterface, MscStatefulRuleInterface
{
    private const CODE = 'D2_00002';

    private const ANEXO_DCA_I_HI = 'DCA-Anexo I-HI';

    private const COD_CONTA_VPD_FUNDEB = '3.5.2.2.0.00.00';

    private const MES_ENCERRAMENTO = 13;

    private const MENSAGEM_INCONSISTENCIA = 'não foi informado, no Anexo I-HI da DCA, valor da variação patrimonial diminutiva com o FUNDEB.';
    ...
    public function finalizeFile(): ?string
    {
        ...
        if ($this->possuiValorVpdFundebValido($items)) {
            return null;
        }
     ...
     */
    private function possuiValorVpdFundebValido(array $items): bool
    {
        foreach ($items as $item) {
            if (! $this->isContaVpdFundeb($item)) {
                continue;
            }
     ...
     */
    private function isContaVpdFundeb(array $item): bool
    {
        ...
        if (is_string($codConta)) {
            ...
            if ($codContaNormalizado === self::COD_CONTA_VPD_FUNDEB) {
                return true;
            }
        }
        ...
        if (is_string($conta) && str_starts_with($conta, self::COD_CONTA_VPD_FUNDEB)) {
            return true;
        }
